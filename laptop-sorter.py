...
start_data_entry = True # Set to True to start data entry process
...
def createqr(laptop_info):
    laptop_uuid = str(laptop_info['UUID'])

    img = qrcode.make(laptop_uuid)

    img.save(f'{laptop_uuid}.png')


def data_entry():
    laptop_info = laptop_data()

    createqr(laptop_info)
